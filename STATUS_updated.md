...
Stand: 2026-09-15
Referenz-Commit: `570e33e3bca933e5adb92e8842025e0f4b914efc`
...
- Schema: `6`
- Getesteter Referenz-Code: `570e33e3bca933e5adb92e8842025e0f4b914efc`
...
### Print-Design -- Ausgabekarten bestätigt, Kopf-Feinschliff aktuell
...
Der finale Buttonstandard und die Ausgabekarten wurden auf dem Android-Gerät ausdrücklich bestätigt.

Aktueller Kopf-Feinschliff in Commit `d663848a93b09e104c722d11eae1b90d8d9cd5bc`:
- Print-Reihenkopf und Rubrikkopf sollen optisch dieselbe kompakte Struktur verwenden;
- im Kopf stehen jeweils Kontextname mit Auswahlpfeil, `Bearbeiten` und `×`;
- `＋ Reihe` bzw. `＋ Rubrik` bleiben oben neben der Suche und erzeugen neue übergeordnete Objekte;
- `＋ Ausgabe hinzufügen` steht im Print-Inhaltsbereich unter dem Reihenkopf, analog zu `＋ Beitrag hinzufügen` unter dem Rubrikkopf;
- eine bestehende Print-Reihe kann nun über `Bearbeiten` im Reihenkopf bearbeitet werden;
- der bestehende Reihen-Auswahlschalter über `Reihenname ▼` bleibt erhalten;
- die bestätigten Print-Ausgabekarten bleiben unverändert.

Dieser letzte Print-Kopf-Feinschliff ist hochgeladen, aber noch nicht separat auf dem Android-Gerät bestätigt.
...
- Der Print-Reihenkopf ist an den bestätigten kompakteren Rubrikkopf angeglichen; nächster visueller Bereich ist Titel.
...
1. Aktuellen Build zu Commit `d663848a93b09e104c722d11eae1b90d8d9cd5bc` als UPDATE installieren und ausschließlich den Print-Kopf-Feinschliff testen: Reihenkopf optisch wie Rubrikkopf, `Bearbeiten` öffnet Reihenbearbeitung, `×` bleibt Löschaktion, `＋ Ausgabe hinzufügen` steht unter dem Kopf, Reihenwahl über `Reihenname ▼` funktioniert weiterhin.
2. Bei erfolgreichem Gerätetest diesen Commit als getestet dokumentieren.
3. Danach Titel visuell auf dieselbe gemeinsame Kopf-/Karten-Designsprache übertragen, ohne Titel-Logik oder Schema zu verändern.
4. Erst anschließend Etappe 6b: Print `Ausgabe → Beitrag → Galerie/Video`, danach Seiten/Fundstellen.

Wichtig für den neuen Chat: zuerst `AGENTS.md` und diese `STATUS.md` vollständig lesen, Referenz-Commit gegen `main` prüfen und erst danach weiterarbeiten.
